...
export default function CourseManager() {
  ...
  const getTotalProgramCredits = (semesters: ISemester[]) => {
    return semesters.reduce(
      (total, semester) => total + getTotalCreditsBySemester(semester.courses),
      0
    );
  };
    ...
    <div className="container max-w-2xl mx-auto p-6">
      <header className="mb-8 text-center">
        ...
        <p className="text-muted-foreground">
          Selecciona las materias que ya has aprobado (Ing.Civil)
        </p>
      </header>
        ...
        <CardHeader>
          <CardTitle className="flex flex-col sm:flex-row gap-4 sm:gap-0 items-center justify-between">
            ...
            <span className="text-lg font-semibold">
              Total de créditos: {getTotalCreditsApproved(coursesSelected)} /{" "}
              {getTotalProgramCredits(semesters)}
            </span>
          </CardTitle>
        </CardHeader>
      </Card>
      ...
      <div className="mb-6 flex items-center justify-center gap-2">
        <Progress
          value={
            (getTotalCreditsApproved(coursesSelected) * 100) /
            getTotalProgramCredits(semesters)
          }
          className="w-[60%]"
        />
                ...
                <CardContent className="p-4">
                  <div className="grid gap-4">
                    {semester.courses.map((course, index) => (
                      <div
                        key={index}
                        className={`flex items-center justify-between p-2 gap-4 cursor-pointer rounded-md hover:bg-accent transition-colors ${
                          coursesSelected.includes(course) ? "bg-accent" : ""
                        }`}
      ...
      <footer className="mt-8 text-center text-muted-foreground">
        <p className="text-sm">Made in China</p>
      </footer>
    </div>
  );
}
